...
namespace cherrydev
{
    public class DialogBehaviour : MonoBehaviour
    {
        ...
        public event Action OnNewNodeActive;

        private DialogNodeGraph _currentNodeGraph;
        public DialogNodeGraph GetCurrentNodeGraph => _currentNodeGraph;
        ...
        public event Action<string> OnSentenceNodeActiveWithParameter;
        public event Action<AnswerNode> OnAnswerNodeActive;
        ...
        public event Action<int, string, AnswerNode> OnAnswerNodeSetUp;
        public event Action OnDialogTextCharWrote;
        public event Action OnDialogTextSkipped;
        public event Action OnSentenceWritingFinished;
        ...
        public void AddListenerToDialogFinishedEvent(Action action)
        {
            OnDialogueFinished += action;
        }
        ...
        private void HandleAnswerNode(Node currentNode)
        {
            ...
            OnAnswerNodeActive?.Invoke(answerNode);
            OnNewNodeActive?.Invoke();
            ...
            for (int i = 0; i < answerNode.ChildSentenceNodes.Count; i++)
            {
                ...
                {
                    OnAnswerNodeSetUp?.Invoke(i, answerNode.Answers[i], answerNode);
        ...
        private void WriteDialogText(string text)
        {
            ...
            _writeTextCoroutine = StartCoroutine(WriteDialogTextRoutine(text));
        }
        ...
        private IEnumerator WriteDialogTextRoutine(string text)
        {
            ...
            OnSentenceWritingFinished?.Invoke();
            yield return Helpers.Helper.GetWait(0.1f);
