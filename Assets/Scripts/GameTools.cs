...
namespace UHFPS.Tools
{
    public static class GameTools
    {
        ...
        /// Play OneShot Audio Clip 3D.
        /// </summary>
        public static AudioSource PlayOneShot3D(bool isLinear, Vector3 position, AudioClip clip, float maxDistance, float volume = 1f, string name = "OneShotAudio")
        {
            if (clip == null)
                return null;
            ...
            source.rolloffMode = isLinear ? AudioRolloffMode.Linear : AudioRolloffMode.Logarithmic;
            source.Play();
            Object.Destroy(go, clip.length * ((Time.timeScale < 0.01f) ? 0.01f : Time.timeScale));
            return source;
        }
